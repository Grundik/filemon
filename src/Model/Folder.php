...
class Folder extends Entry {
  ...
  protected function _checkList($current, $saved, $level) {
    foreach ($current as $entry) {
      $k = $this->_findEntry($saved, $entry);
      if (!$k) {
        \Filemon\printLine("New entry: {$this->root_path}/{$entry->getName()}", 0, 1);
        continue;
      }
      $k->_found = true;
      ...
      if ($k->getDeleted()) {
        \Filemon\printLine("Entry undeleted: {$this->root_path}/{$k->getName()}", 0, 1);
      }
    }
    foreach ($saved as $f) {
      if (!$f->_found && !$f->getDeleted()) {
        \Filemon\printLine("Entry disappeared: {$this->root_path}/{$f->getName()}", 0, 1);
      }
    }
  }
